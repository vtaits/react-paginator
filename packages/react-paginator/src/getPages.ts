...
import type { GetPages, PagesBlock } from "./types";
...
export const getPages: GetPages = ({
	...
	let startPageInMainRange: number = Math.max(
		1,
		Math.ceil(page - pageRangeDisplayed / 2),
	);
	let endPageInMainRange: number =
		startPageInMainRange + pageRangeDisplayed - 1;
